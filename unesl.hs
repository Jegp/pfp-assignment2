...
Version 0.06, 4 December 2017

This is a simplistic interpreter for a subset of the NESL language,
instrumented to also compute work and span costs of the expressions
...
The main function is "nesl". Top level commands are of the form:
  ...
  exp                         Type-check and evaluate the NESL expression
  function f(x1..xn)[:type] = exp  Define a NESL function
  def x = exp                 Define a NESL value
  :load filename.nesl         Read function/value defs from file
  :defs                       Show types of all currently defined identifiers
...
Changelog:
 ...
 0.03   Added typing of functions (for compatibility; ignored for now)
 0.04   Adapted for GHC 7.10 Applicative => Monad brain damage.
 0.05   Minor tweaks
 0.06   Added :defs command
-}
        ...
        (case rComp (eval e vr) "TopExp" of
           Left (v, nw, ns) -> 
              do putStrLn (show v ++ " : " ++ show pt)
                 putStrLn ("[Work: " ++ show nw ++ ", span: " ++ show ns ++ "]")
...
doTop (TLoad fn) r =
  do s <- C.catch (readFile fn)
                  (\e -> print (e::IOError) >> return "")
     case parse parseDefs fn s of
       Right ds -> doDefs ds r
       Left err -> print err >> return r
doTop TDump r =
  do mapM_ (\(i,t) -> putStrLn (i ++ " : " ++ show t)) (snd r)
     return r
...
-- comment out starting here if you don't have Haskeline
